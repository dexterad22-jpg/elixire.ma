...
function syncToGithub() {
    ...
    const api = `https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/admin_products.json`;
    ...
    fetch(api, { headers })
        ...
        .then(data => {
            const body = {
                message: 'Sauvegarde des produits ajoutés',
        ...
        .then(r => {
            if (r.ok) {
                if (status) { status.textContent = '✅ Sauvegardé sur GitHub !'; status.className = 'admin-token-status ok'; }
            } else {
                if (status) { return r.json().then(e => { throw new Error(e.message); }); }
            }
        })
        .catch(err => {
            if (status) { status.textContent = '❌ Erreur: ' + err.message; status.className = 'admin-token-status err'; }
        });
}

function fetchFromGithubJson() {
    return new Promise(function(resolve) {
        const token = getGithubToken();
        if (!token) { resolve(null); return; }
        const api = 'https://api.github.com/repos/' + GITHUB_OWNER + '/' + GITHUB_REPO + '/contents/admin_products.json';
        fetch(api, { headers: { Authorization: 'Bearer ' + token, Accept: 'application/vnd.github.v3+json' } })
            .then(function(r) {
                if (!r.ok) { resolve(null); return; }
                return r.json();
            })
            .then(function(data) {
                if (!data || !data.content) { resolve(null); return; }
                try {
                    var decoded = b64Decode(data.content);
...
function loadProducts() {
    var adminData = getAdminProducts();
    if (adminData && adminData.length) {
        mergeProducts(adminData);
    } else {
        var self = this;
        fetchFromGithubJson().then(function(githubData) {
            if (githubData && githubData.length) {
                saveAdminProductsLocal(githubData);
                mergeProducts(githubData);
                renderFeatured();
                renderCatalogue();
                updateCartUI();
            }
...
function saveAdminProductsLocal(data) {
    localStorage.setItem('dareloutour_admin_products', JSON.stringify(data));
}
